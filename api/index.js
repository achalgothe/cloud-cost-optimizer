...
const {
  ...
  mockResources,
  mockBudgets,
} = require('../server/mock-data');

// Import services
const CostAnalyticsService = require('../server/services/costAnalyticsService');
const AnomalyDetectionService = require('../server/services/anomalyDetectionService');
...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'OK',
    message: 'Cloud Cost Optimizer API (Vercel)',
    mode: 'mock-data'
  });
});
...
app.put('/api/resources/:id', protect, (req, res) => {
  const { id } = req.params;
...
// Data flow route
app.get('/api/data/flow', protect, (req, res) => {
  res.json({
    success: true,
    message: 'Data flow endpoint',
  });
});
